...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON games.winner_id=teams.team_id WHERE winner_goals >= 1 ORDER BY name ASC")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM teams FULL JOIN games ON games.winner_id=teams.team_id WHERE round='Final' ORDER BY name DESC")"
